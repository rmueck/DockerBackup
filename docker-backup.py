...
def main():
    ...
    tar_args = ["tar", "--use-compress-program=pigz", "-cf", temp_backup_path, DOCKER_VOLUME_DIR] + config_files
    run(tar_args)
    if DRY_RUN:
    ...
    for dir_to_backup in ADDITIONAL_DIRECTORIES_TO_BACKUP:
        ...
        try:
            ...
            run(["tar", "--use-compress-program=pigz", "-cf", temp_backup_path, dir_to_backup], check=True)
            if DRY_RUN:
